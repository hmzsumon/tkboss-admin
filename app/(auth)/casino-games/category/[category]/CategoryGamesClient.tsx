...
/* ────────── app/(public)/casino-games/category/[category]/CategoryGamesClient.tsx ─────────── */
...
export default function CategoryGamesClient({
  ...
  const activeCatTitle = catRes?.items?.find(
    ...
    <main className="mx-auto w-full max-w-7xl p-4 md:p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">
          {activeCatTitle || categorySlug}
        </h1>
        <p className="mt-1 text-sm text-neutral-500">
          Showing games by category.
        </p>
      </div>
      ...
      <IgamingInfiniteGrid category={categorySlug} q={debounced} />
    </main>
  );
}
